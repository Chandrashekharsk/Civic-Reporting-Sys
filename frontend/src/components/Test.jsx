...
const reportSchema = z.object({
  ...
  category: z.enum(["pothole", "streetlight", "trash", "other"]),
...
export default function ReportForm() {
  ...
  const [submittedReport, setSubmittedReport] = useState(null); // full report
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      setSubmittedReport(res); // store full report object
      reset();
      setCoords(null);
      setAutoCity("");
  ...
  // If submitted, show report details instead of form
  if (submittedReport) {
    return (
      <div className="max-w-lg mx-auto mt-6 p-6 bg-white rounded-2xl shadow-md">
        ...
        <div className="mt-4 p-4 bg-green-100 border border-green-300 rounded-lg">
          <p className="text-gray-800 font-medium mb-2">
            ...
            <span className="font-bold text-indigo-700">
              {submittedReport._id || submittedReport.id}
            </span>
          </p>
          ...
          <p className="mt-2 text-sm text-red-600 font-medium text-center">
            ⚠️ Ensure you have copied the Report ID.
          </p>
        </div>
        ...
        {submittedReport.image && (
          <div className="mt-4">
            ...
            <img
              src={submittedReport.image} // assuming backend returns image URL
              alt="Report"
              className="mt-2 rounded-lg border shadow-md"
            />
          </div>
        )}
      </div>
  ...
  return (
    <form
      ...
      className="max-w-md mx-auto mt-6 p-6 bg-white rounded-2xl shadow-md space-y-4"
    >
